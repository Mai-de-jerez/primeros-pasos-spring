...
public class PerfilController {
	...
	/*
	 * public String procesarEdicion(@AuthenticationPrincipal CustomUserDetails
	 * userDetailsActual,
	 * 
	 ...
	 * 
	 * log.info("Procesando actualización de perfil para el usuario: '{}'",
	 * userDetailsActual.getUsername());
	 * 
	 * 
	 * // validaciones de contraseñas if (!usuarioDto.password().isEmpty()) { if
	 * (!usuarioDto.password().matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,}$")) {
	 ...
	 * 
	 * if (bindingResult.hasErrors()) { return "usuario/editar-perfil"; }
	 * 
	 * try { // Actualizamos db User actualizado =
	 * userService.actualizarPerfil(userDetailsActual.getUsername(), usuarioDto);
	 * CustomUserDetails nuevoUserDetails = new CustomUserDetails(actualizado);
	 * 
	 * // actualizamos e contexto var auth = new
	 * UsernamePasswordAuthenticationToken( nuevoUserDetails, null,
	 ...
	 * 
	 * log.info("Perfil y sesión de Redis actualizados con éxito para '{}'",
	 * actualizado.getUsername()); return "redirect:/perfil?actualizado";
	 * 
	 * } catch (RuntimeException e) { log.warn("Error al actualizar perfil: {}",
	 * e.getMessage()); model.addAttribute("error", e.getMessage()); return
